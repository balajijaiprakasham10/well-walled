...
const GalleryPage: React.FC = () => {
    ...
    return (
        // 1. REMOVED bg-[#f4f4f4] from here. 
        // We need the background to be transparent so the -z-10 banner shows through.
        <div className="min-h-screen text-black font-sans selection:bg-black selection:text-white overflow-x-hidden">

            {/* HERO BANNER — FULL BLEED */}
            {/* Removed the negative margin hack. Since we are at the top level, we don't need it if we structure layout correctly */}
            <div className="w-full">
                <HeroBanner
                    page="gallery"
                    title="Gallery"
                    subtitle="A collection of our curated design experiences."
                    className="h-screen" // Adjust height as needed, full screen might be too much for gallery
                />
            </div>

            {/* Main Content Wrapper */}
            {/* 2. ADDED bg-[#f4f4f4] here. 
               This ensures the background starts ONLY where the content starts, 
               creating the effect of content scrolling over the fixed banner. */}
            <div
                className={`
                    relative z-10 bg-[#f4f4f4]
                    px-6 md:px-12 py-20 
                ...
                {projects.map((project, index) => {
                    ...
                    return (
                        <div key={projectId} className="mb-40 last:mb-0 group">
                            ...
                            {/* Layout: Alternating Image & Details */}
                            <div
                                className={`
